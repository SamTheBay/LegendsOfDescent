...
{
    class MoltenEarthAbility : Ability
    {
        int[] damage = { 2, 5, 8, 25, 45, 70 };
        ...
        public MoltenEarthAbility(PlayerSprite player)
            : base("Molten Earth", player, 23)
        {
            manaCost[0] = 30;
            manaCost[1] = 40;
            manaCost[2] = 50;
            ...
            isMagic = true;
            abilityType = AbilityType.MoltenEarth;
        ...
        public override string Description
        {
            get
            {
                ...
                if (level != 0)
                    offset = level - 1;
                return "Turns an area on the ground to molten lava causing enemies that walk on it to be burnt for " + damage[offset].ToString() + " fire damage per second.";
            }
        }
        ...
        public override string NextDescription
        {
            get
            {
                ...
                return "Increase the damage dealt to " + damage[level - 1].ToString() + ".";
            }
        }
        ...
        public override void TileAbilityEfffect(CharacterSprite sprite)
        {
            ...
            if (sprite is EnemySprite)
            {
                ...
                if (!alreadyDamagedEnemies.Contains(enemy) && !enemy.IsDead)
                {
                    ...
                    enemy.Damage(damage[level - 1] + player.SpellDamage / 10, DamageType.Fire, player);
                    ParticleSystem.AddParticles(enemy.CenteredPosition, ParticleType.Explosion, sizeScale: 1f, lifetimeScale: 1f, numParticlesScale: 1.0f);
                }
            }
        }
    }
