...
{
    class EnchanterScreen : PopUpScreen
    {
        static int boarderWidth = 6;
        PlayerSprite player;
        EnchanterSprite enchanter;
        Texture2D blankSlot;
        ...
        public EnchanterScreen(PlayerSprite player, EnchanterSprite enchanter)
            : base()
        {
            ...
            arrow = DungeonGame.Instance.Content.Load<Texture2D>("Textures/UI/ArrowRight");
            ...
            if (DungeonGame.portraitMode && !DungeonGame.pcMode)
            {
                ...
                upgradeSlotsCorner.Y = WindowCorner.Y + 140 + 50;
                ...
                entry.Position = new Vector2(upgradeSlotsCorner.X - 15, upgradeSlotsCorner.Y - 80);
                entry.Selected += new EventHandler<EventArgs>(entry_Selected);
                MenuEntries.Add(entry);

                entry = new MenuEntry("");
                entry.Texture = arrow;
                entry.Position = new Vector2(upgradeSlotsCorner.X + arrow.Width - 15, upgradeSlotsCorner.Y - 80);
                entry.Selected += new EventHandler<EventArgs>(entry_Selected);
                MenuEntries.Add(entry);
            }
            else
            {
                ...
                upgradeSlotsCorner.Y = WindowCorner.Y + dimension.Height / 2 - blankSlot.Height / 2 + 50;
        ...
        void entry_Selected(object sender, EventArgs e)
        {
            if (selectedEntry == 0)
            {
                if (upgradeable && upgradeSlot[0, 0] != null && player.Gold >= upgradeCost)
                {
                    ...
                    item.AdjustModifier(modifier.property, modifier.amount);
            ...
            {
                enchanter.MoveActiveProperty(true);
                SetUpgradeDetails();
            }
            else if (selectedEntry == 2)
            {
                enchanter.MoveActiveProperty(false);
                SetUpgradeDetails();
            }
        }
        ...
        public override void Draw(Microsoft.Xna.Framework.GameTime gameTime)
        {
            ...
            // draw title
            Fonts.DrawCenteredText(spriteBatch, Fonts.DCFont, enchanter.Name, titleVector, Color.Red);
            ...
            if (upgradeSlot[0, 0] != null)
            {
                ...
                {
                    foreach (string descriptionLine in Fonts.BreakTextIntoList(upgradeDescription[i], Fonts.DescriptionFont, 200))
                    {
                        spriteBatch.DrawString(Fonts.DescriptionFont, descriptionLine, location, Color.White);
                        location.Y += 20;
                    }
                    location.Y += 10;
                }
        ...
        public void SetUpgradeDetails()
        {
            ...
            if (item is EquipableItem)
            {
                ...
                // build property
                if (enchanter.ActiveProperty == Property.AttackSpeed || enchanter.ActiveProperty == Property.CastSpeed || enchanter.ActiveProperty == Property.MoveSpeed)
                {
                    modifier = new PropertyModifier(enchanter.ActiveProperty, 2);
                }
                else
                {
                    modifier = new PropertyModifier(enchanter.ActiveProperty, 3);
                }
                modifier = ItemManager.Instance.TranslateProperty(modifier, ei.Level, ei.EquipSlot);
                ...
                {
                    upgradeDescription.Add(ItemManager.PropertyToString(modifier.property) + " +" + modifier.amount.ToString() + "%");
                }
                else
                {
